...
class MultiBandMRIDataset(torch.utils.data.Dataset):
    ''' Multiband MRI data set '''

    def __init__(self, data_file, stdev=.01, num_data_sets=None, adjoint=True, preload=False, id=None, clear_cache=False, cache_data=False, gen_masks=False, scale_data=False, fully_sampled=False, data_idx=None, inverse_crime=False, noncart=True):
    ...
    def __getitem__(self, idx):
        ...
        if self.cache_data and self.id:
            ...
            try:
                imgs, maps, masks, phi, out = load_data_cached(data_file)
            except:
                imgs, maps, masks, phi, out = self._load_data(idx)
                save_data_cached(data_file, imgs, maps, masks, phi, out)
        else:
                imgs, maps, masks, phi, out = self._load_data(idx)
        ...
        data = {
                ...
                'phi': cp.c2r(phi.squeeze()).astype(np.float32),
    ...
    def _load_data(self, idx):
        if self.inverse_crime:
            ...
            imgs, maps, phi, masks = load_data(idx, self.data_file, self.gen_masks)
        else:
            imgs, maps, masks, phi, ksp = load_data_ksp(idx, self.data_file, self.gen_masks)
        ...
        if self.inverse_crime:
            ...
            out = self._sim_data(imgs, maps, masks, phi)
        else:
            out = self._sim_data(imgs, maps, masks, phi, ksp)

        if not self.noncart:
            maps = fftmod(maps)
        return imgs, maps, masks, phi, out

    def _sim_data(self, imgs, maps, masks, phi, ksp=None):
...
def load_data_ksp(idx, data_file, gen_masks=False):
    ...
    if len(masks.shape) == 2:
        imgs, maps, masks, phi, ksp = imgs[None,...], maps[None,...], masks[None,...], phi[None,...], ksp[None,...]
    return imgs, maps, masks, phi, ksp
...
def save_data_cached(data_file, imgs, maps, masks, phi, out):
